    ...
    <div className="quarter-card" data-status={status}>
      ...
      {goal ? (
        <>
          ...
          <WaypointList waypoints={goal.waypoints} isActiveQuarter={status === 'active'} locale={locale} />
        </>
      ) : (
        <div className="quarter-card__placeholder">Plan during {activeQuarterLabel} review →</div>
      )}
    </div>
  );
}
