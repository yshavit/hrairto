...
export default function ActivityCard({ goal, quarter, concernColor, status, locale }: Props) {
  ...
  return (
    <div className="activity-card" data-status={status} style={{ '--concern-color': concernColor } as CSSProperties}>
      ...
      <WaypointList waypoints={goal.waypoints} quarter={quarter} isActiveQuarter={status === 'active'} locale={locale} />
    </div>
  );
}
