...
function trimQuartersForGoal(goal: AnnualGoal, quarters: QuarterDisplay[]): QuarterDisplay[] {
  const deadlineIdx = quarters.findIndex((q) => q.quarter === goal.due_quarter && q.year === goal.due_year);
  if (deadlineIdx === -1) return quarters;
  return quarters.slice(0, deadlineIdx + 2);
}

const SwimlanesContainer = forwardRef<ScrollAPI, Props>(function SwimlanesContainer({ swimlanes, annualGoals, quarterlyGoals, quarters, locale }, ref) {
    ...
    return (e) => {
      ...
      const onMove = (moveE: PointerEvent) => {
        ...
        if (rawScroll > maxScroll) {
          ...
          isSyncing.current = true;
          scrollerRefs.current.forEach((r) => {
            if (r) r.scrollLeft = maxScroll;
          });
          innerRefs.current.forEach((r) => {
            if (r) r.style.transform = `translateX(${-rubberOffset}px)`;
          });
          requestAnimationFrame(() => {
            isSyncing.current = false;
          });
          ...
          isSyncing.current = true;
          scrollerRefs.current.forEach((r) => {
            if (r) r.scrollLeft = Math.max(0, rawScroll);
          });
          requestAnimationFrame(() => {
            isSyncing.current = false;
          });
  ...
  const swimlaneLayouts = swimlanes.map((swimlane) => {
    ...
    const mainQuestGoals = quarterlyGoals.filter((g) => g.swimlane_id === swimlane.id && g.annual_goal.type === 'MainQuest');
    const sideQuestGoals = quarterlyGoals.filter((g) => g.swimlane_id === swimlane.id && g.annual_goal.type === 'SideQuest');
