...
import type { Concern, DistractionLabel, MainQuestPlanningContext, WeeklyGoal, WeeklyGoalId } from '../bindings';
...
function goalMeta(goal: WeeklyGoal, concerns: Concern[], quarterContext: MainQuestPlanningContext[], distractionLabels: DistractionLabel[]): GoalMeta {
  ...
  if (ref.type === 'Planned') {
    ...
    if (ref.waypoint_id) {
      for (const ctx of quarterContext) {
        const wp = ctx.quarterly_goal?.waypoints.find((w) => w !== null && w.id === ref.waypoint_id);
        if (wp) {
          detail = wp.text;
          break;
        }
...
export default function PastGoalsList({ goals, outcomes, onToggle, concerns, quarterContext, distractionLabels }: Props) {
    ...
    <div className="past-goals-list">
      ...
      <ul className="past-goals-list__goals">
        {goals.map((goal) => {
          ...
          const meta = goalMeta(goal, concerns, quarterContext, distractionLabels);
          return (
