...
| `SwimlaneWeight` / `SwimlaneWeightPeriod` / `SwimlanesFocus`   | `WeightEntry` / `FocusTarget` / `Focus`                                                                      | drop the now-wrong `Swimlane` prefix; `WeightEntry`'s field `target` → `activity`; also `SwimlaneWeightPeriodId` → `FocusTargetId`          |
